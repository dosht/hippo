...
**Reconcile** is deferred post-MVP. The full reconcile responsibilities (confirms / contradicts / extends / obsoletes / feedback processing / promotion candidates / staleness) are sketched in `docs/brainstorm/reconcile-design.md` (design-only; no `scripts/reconcile.py` exists) and listed individually in the MVP-1 epic's Out-of-Scope section.

**Deduplication boundary** (MVP-2-06): semantic deduplication belongs exclusively in the reconcile cold path, which has full entry bodies and corpus-wide context. `extract.py` uses only exact-id collision detection: if `(gold_dir / f"{entry_id}.md").exists()` the entry is skipped, otherwise it is written. This is deterministic, corpus-size-independent, and produces no false positives. Semantic similarity scoring at extract time caused a silent corpus freeze (Apr 28 -- May 1 2026) as the gold corpus grew to cover common topic domains and every new entry matched an existing one above the 0.85 threshold.

**Recovery flag**: `extract.py --recover-empty-gold` re-processes manifest entries with `status: "gold"` and `gold_paths: []` using the same `extract_session()` flow. Entries that genuinely produced no knowledge write `gold_paths: []` again (harmless). Entries that were silently dropped by the old semantic dedup gain non-empty `gold_paths`. Running this after the fix deployed grew the corpus from 29 to 101 entries; 40 sessions remain with `gold_paths: []` because the model found no extractable knowledge in them (legitimately empty).

**AC4 monitoring**: at the end of `main()`, `extract.py` reads `manifest_path` and emits a `WARNING`-level log (via the existing stdlib logger) if any entry with `status: "gold"` and `gold_paths: []` has an `extracted_at` timestamp within the last 24 hours. This surfaces a recurrence of this class of regression within one nightly run.
